...
export const useImpressoes = () => {
  ...
  if (sortColumns.length > 0) {
    sortedImpressoes.sort((a, b) => {
      for (const sort of sortColumns) {
        ...
        if (sort.column === "dataInicio" || sort.column === "dataConclusao" || sort.column === "createdAt") {
  ...
  const dateFilteredImpressoes = filterByDateRange(filteredImpressoes, startDate, endDate, "dataInicio");
